...
/
set termout on
select 'C:\Ganesh\Work\SQL-SPOOL\'||'&instance'||'mon_delphix_rman_'||to_char(sysdate,'yyyymmdd') spoolfile from dual
/
spool &spoolfile
...
select inst_id,sid,serial#,username,logon_time,program,status,osuser,
...
select s.inst_id, s.sid sid, p.spid spid, s.machine,s.client_info client_info
