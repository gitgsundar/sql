-- 	File 		:	GET_TABLE_STATS_INFO.SQL
--	Description	:	Provides Details of Table Statistics
...
col partition_name		heading 'Part Name'		format a25
...
select 'C:\Ganesh\Work\SQL-SPOOL\'||'&instance'||'get_table_stat_info_'||to_char(sysdate,'yyyymmdd-hh24miss') spoolfile from dual
/
spool &spoolfile
...
tti Left 'Stats History Information :' skip 2

select table_name, partition_name, subpartition_name,stats_update_time
...
select table_name, partition_name,num_rows,avg_row_len,blocks,sample_size,last_analyzed
...
select table_name, partition_name,num_rows,blocks,empty_blocks,chain_cnt,avg_row_len,sample_size,last_analyzed,user_stats,stale_stats
...
tti off
tti Left 'Table Columns Stats Information :' skip 2

select table_name, owner, column_name, notes, histogram
